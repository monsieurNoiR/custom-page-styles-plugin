...
- **1.0.2** (2024-12-18): スタイル読み込み優先度の最適化
  - wp_enqueue_scripts フックの優先度を20に設定し、テーマCSSより後に読み込むように改善

- **1.0.1** (2024-12-18): セキュリティ強化パッチ
  - CSSサニタイゼーションの大幅強化（WP_Error対応、追加の危険パターン検出）
  - ファイルサイズ制限追加（1MB）
  - エラーハンドリングの改善（Transient API使用）
  - ファイルパス検証の強化
  - イベントハンドラー（onclick, onerror等）の検出追加
  - data:text/html, vbscript:等の検出追加
